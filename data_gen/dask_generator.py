  ...
  for cword, tword in zip(camera_names, logs_names):
    try:
      with h5py.File(tword, "r") as t5:
        ...
        #print(angle[-1])
        goods = np.abs(angle[-1]) <= 500

        filter_index = np.argwhere(goods)[time_len-1:]
        
        filters.append(filter_index[:,0] + (lastidx))
...
def datagen(filter_files, time_len=1, batch_size=256, ignore_goods=False):
  ...
  X_batch = np.zeros((batch_size, time_len, 160, 800, 3), dtype='uint8')
  angle_batch = np.zeros((batch_size, 1, 1), dtype='float32')
  speed_batch = np.zeros((batch_size, 1, 1), dtype='float32')
  ...
  while True:
    try:
      ...
      while count < batch_size:
        ...
        for es, ee, x in c5x:
          if i > es and i <= ee:
            X_batch[count] = x[i-es-time_len:i-es]
            break

        #print((angle[i-time_len+1:i+1]).shape)
        angle_batch[count] = np.copy(angle[i])#[:, None]

        speed_batch[count] = np.copy(speed[i])#[:, None]

        count += 1

      # sanity check
      #assert X_batch.shape == (batch_size, time_len, 160, 320, 3)
      assert X_batch.shape == (batch_size, time_len, 160, 800, 3)
